@extends('layouts.master')
@section('content')
    <div class="rounded-xl border border-gray-300 relative overflow-x-auto w-[80%] my-10 mx-auto" style="direction: ltr">
        <div class="flex justify-between border-b border-b-gray-300 p-4 bg-gray-100">
            <span class="font-bold text-xl">Edit Todo</span>
            <a href="{{ route('todo.index') }}"
                class="rounded-sm bg-gray-800 hover:bg-gray-600 transition-all text-white w-22 text-center h-8 pt-1">back</a>
        </div>
        <form class="m-5" action="{{route('todo.update' , ['todo' => $todo->id])}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')

            <div class="mb-2">Image Now</div>
            <div class="w-40 border border-gray-200 rounded-md">
                <img class="rounded-b-md w-40" src="{{ asset('images/' . $todo->image) }}" alt="" />
            </div>

            <div>Image</div>
            <input name="image" type="file"
                class="input-create mt-3 w-full h-9 border border-gray-200 rounded-md focus-visible:outline-gray-300 pl-2" />
            <div class="text-sm text-red-700 mb-5 mt-2">
                @error('image')
                    {{ $message }}
                @enderror
            </div>
            <div>Title</div>
            <input name="title" type="text" value="{{ $todo->title }}"
                class="input-create mt-3 w-full h-9 border border-gray-200 rounded-md focus-visible:outline-gray-300 pl-2" />
            <div class="text-sm text-red-700 mb-5 mt-2">
                @error('title')
                    {{ $message }}
                @enderror
            </div>
            <div>Category</div>
            <select name="category_id"
                class="input-create mt-3 w-full h-9 border border-gray-200 rounded-md focus-visible:outline-gray-300 pl-2">
                @foreach ($categories as $category)
                    <option {{ $todo->category_id == $category->id ? 'selected' : '' }} value="{{ $category->id }}">
                        {{ $category->title }}</option>
                @endforeach
            </select>
            <div class="text-sm text-red-700 mb-5 mt-2">
                @error('category_id')
                    {{ $message }}
                @enderror
            </div>
            <div>Description</div>
            <textarea name="description" rows="10"
                class="input-create mt-3 w-full h-24 border border-gray-200 rounded-md focus-visible:outline-gray-300 pl-2">{{ $todo->description }}</textarea>
            <div class="text-sm text-red-700 mb-5 mt-2">
                @error('description')
                    {{ $message }}
                @enderror
            </div>
            <button type="submit"
                class="rounded-lg p-2 transition-all bg-gray-500 text-white hover:bg-gray-400 text-center">Submit</button>
        </form>
    </div>
@endsection
